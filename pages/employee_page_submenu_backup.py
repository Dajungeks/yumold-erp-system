...
def show_employee_page(manager, auth_manager, user_permissions, get_text):
    ...
    if selected_submenu == "직원 목록":
        show_employee_list(manager)
    elif selected_submenu == "직원 등록":
        show_employee_registration(manager)
    elif selected_submenu == "직원 편집":
        show_employee_edit(manager)
    elif selected_submenu == "직원 통계":
        show_employee_statistics(manager)
    elif selected_submenu == "직원 권한":
        show_employee_permissions(manager, auth_manager)
    elif selected_submenu == "직원 대량 등록":
        show_employee_bulk_registration(manager)
    else:
        st.warning(f"선택된 서브메뉴 '{selected_submenu}'에 대한 페이지가 준비되지 않았습니다.")

def show_employee_list(manager):
    """직원 목록 표시"""
    st.header("📋 직원 목록")
...
def show_employee_registration(manager):
    """직원 등록 폼 표시"""
    st.header("➕ 새 직원 등록")
...
def show_employee_edit(manager):
    """직원 편집 페이지"""
    st.header("✏️ 직원 편집")
...
def show_employee_statistics(manager):
    """직원 통계 페이지"""
    st.header("📊 직원 통계")
...
def show_employee_permissions(manager, auth_manager):
    """직원 권한 관리 페이지"""
    st.header("🔐 직원 권한 관리")
    st.info("권한 관리 기능은 개발 중입니다.")

def show_employee_bulk_registration(manager):
    """직원 대량 등록 페이지"""
    st.header("📤 직원 대량 등록")
    st.info("대량 등록 기능은 개발 중입니다.")